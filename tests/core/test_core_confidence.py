...
class TestConfidenceCalculator:
    ...
    def mock_gene(self):
        ...
        from helixforge.core.gff import GeneModel, TranscriptModel
...
# =============================================================================
# Minus-strand coverage (CLAUDE.md §12: both strands mandatory)
# =============================================================================


class TestMinusStrand:
    """score_gene on a minus-strand gene (mirror of TestConfidenceCalculator)."""

    @pytest.fixture
    def mock_reader(self):
        """Mock HDF5 reader; records the strand it was asked for."""
        reader = MagicMock()
        reader.scaffold_names = ["chr1"]
        reader.scaffold_lengths = {"chr1": 1000}
    ...
    def test_score_gene_minus_strand(self, mock_reader, minus_gene) -> None:
        """Scoring a minus-strand gene yields valid metrics and passes strand."""
        calc = ConfidenceCalculator(mock_reader)
        score = calc.score_gene(minus_gene)
        ...
        # The reader must be queried with the gene's strand ("-").
        strands = {
            call.kwargs.get("strand", call.args[3] if len(call.args) > 3 else "+")
            for call in mock_reader.get_predictions_for_region.call_args_list
        }
        assert strands == {"-"}
