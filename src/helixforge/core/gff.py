"""GFF3/GTF file handling for the standalone confidence scorer."""
        ...
        with open(self.path) as f:
            for line in f:
                ...
                if ftype in FEATURE_TYPES_GENE:
                    gene_id = attrs.get(
                        "ID", attrs.get("gene_id", f"gene_{len(gene_features)}")
                    )
                    ...
                    tx_id = attrs.get(
                        "ID",
                        attrs.get("transcript_id", f"tx_{len(transcript_features)}"),
                    )
